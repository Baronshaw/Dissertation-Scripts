function [] = getMapsFires_CAMP(pah,showhard,showsoft)
% GETMAPS_CAMP visualizes results of all pah/soft data/dimension
% combinations
%
...
if nargin < 3, showsoft = 1; end % show soft on maps
...
% loading estimates
load(sprintf('matfiles/BMEest_%s_CAMP.mat',valname{pah+4}));
...
for i = 1:length(dayShow)
    ...
    title(sprintf('Mean %s CAMP on %s with fire',valdispname{pah},datestr(dayShow(i),2)));
    ...
    print(gcf,'-painters','-dpng','-r600',sprintf('figures/Fire_BMEest_%s_CAMP_%s.png', ...

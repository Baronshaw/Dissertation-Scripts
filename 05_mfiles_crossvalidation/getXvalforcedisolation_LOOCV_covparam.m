function [] = getXvalforcedisolation_LOOCV_covparam(soft,constant,gauss,forceddist)
...
% load different covariance information
load('RegionByID.mat');
...
if soft == 1
    ...
    if gauss == 0, probdens = cell2mat(probdensA_mrmv);
        ...
        % lambda2 = 0.5*lambda2; % added 10/7/2014
    end
...
elseif gauss == 1
    ...
    for i = 1:size(cMS,1)
        ...
        ck = ch(idx & temp2001==2001,:); % added 9/14/2014
        ck = ch(idx & temp2005==2005,:); % added 10/30/2014
        ...
        if soft == 1
            if ~isempty(ck)
                n = EPAuni(i);
                if EPAuni(i) == 25, n = 11; end
                covparam = {[f{n}.Cr1*f{n}.alp f{n}.ar1 f{n}.at1] [f{n}.Cr1*(1-f{n}.alp) f{n}.ar2 f{n}.at2]};
                % dmax = [2000000 365 f{n}.alp*f{n}.ar1/f{n}.at1 + (1-f{n}.alp)*f{n}.ar2/f{n}.at2];
...
% temporary save
save(sprintf('../matfiles/Xvalforcediso_LOOCV_%s%s%s_foriso%dkm_timezone_2005_covparam_fixeddmax.mat',softstr,constr,gaussstr,floor(forceddist./1000)), ...
...
% % all this commented for sake of computational time
...
%         disp([j len]);
%         tic
%         [liA locB] = cellfun(@(x) ismember(x,pII,'rows'),allch{i},'UniformOutput',false);
%         toc
...
% toc
% if soft == 1
%     for i = [2001 2002 2005:2007]
%         load(sprintf('../matfiles/meanTrend_%d_%d_%d_%d_soft_yr%d.mat',[900000 300000 100 50],i));
%         pII{i,1} = pI;
%         mII{i,1} = mI;
%     end
%     pII = cell2mat(pII); mII = cell2mat(mII);
%     tic

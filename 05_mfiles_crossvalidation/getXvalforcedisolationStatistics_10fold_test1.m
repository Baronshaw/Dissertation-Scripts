function [] = getXvalforcedisolationStatistics_10fold_test1(soft,constant,gauss)
...
for i = 1:10   
    % loading results
    load(sprintf('../matfiles/Xvalforcediso_10fold_fold%d%s%s%s_test1.mat',i,softstr,constr,gaussstr));
    zkall{i,1} = zk_madd;
    zhall{i,1} = zh_Xval;
    ckall{i,1} = ck;
    vkall{i,1} = vk;  
...
% save results
save(sprintf('Xvalforcediso_10fold%s%s%s_results_test1.mat',softstr,constr,gaussstr),...

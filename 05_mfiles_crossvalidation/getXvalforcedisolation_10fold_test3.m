function [] = getXvalforcedisolation_10fold_test3(soft,constant,gauss,forceddist,fold)
...
if nargin < 4, forceddist = 500000; end % distance in meters
if nargin < 5, fold = 1; end % 1 out of the 10 folds
...
if soft == 1
    ...
    for i = 1:length(CTMyears) 
        disp(CTMyears(i));
        load(sprintf('../matfiles/PM2p5_soft_yr%d_test3.mat',CTMyears(i)));
        cssA{i,1} = css;
        limiA{i,1} = limi;
...
if gauss == 0
    ...
    ch_nonXvaltempb = ch_nonXval(tempb,:);
    zh_mrmv_nonXvaltempb = zh_mrmv_nonXval(tempb);
    ...
    for i = 1:length(ck)
        ...
        idxsub = D > forceddist; 
        [moments(i,:),info(i,:)]=BMEprobaMoments2(cksub,ch_nonXvaltempb(idxsub,:), ...
            css,zh_mrmv_nonXvaltempb(idxsub),softpdftype,nl,limi,probdens, ...
...
elseif gauss == 1
    ...
    for i = 1:length(ck)
        ...
        D = sqrt(bsxfun(@plus,dot(X,X,2),dot(Y,Y,2)')-2*(X*Y'));
        idxsub = D > forceddist;
        [zk(i,:),vk(i,:),temp1(i,:),temp2(i,:),temp1a(i,:),temp1b(i,:)] = ...
            krigingME2_correct(cksub,ch_nonXval(idxsub,:),css,...
            zh_mrmv_nonXval(idxsub),lambda1,lambda2,covmodel,covparam,nhmax,nsmax,dmax,order,options);
...
% saving data
if gauss == 1
    save(sprintf('../matfiles/Xvalforcediso_10fold_fold%d%s%s%s_test3.mat',fold,softstr,constr,gaussstr), ...

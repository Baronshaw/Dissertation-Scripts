function [] = plotObs(soft,constant,gauss,daydisp)
...
Value ={'o',5,[0 0 0]};
colorplot(ckall(idx,1:2),zhall(idx),'hot',Property,Value);
cax = [prctile(zhall,5) prctile(zhall,95)];
caxis(cax);
colorbar;
title(sprintf('Obs on %d %s %s %s %s',dayshow,softstr(2:end),constr(2:end),gaussstr(2:end))); 
...
print(gcf,'-painters','-dpdf','-r600',sprintf('obs%d_%s%s%s_test1.pdf',dayshow,softstr,constr,gaussstr));
print(gcf,'-painters','-dpng','-r600',sprintf('obs%d_%s%s%s_test1.png',dayshow,softstr,constr,gaussstr));

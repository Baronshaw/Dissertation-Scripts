function [] = getXvalforcedisolation_LOOCV_test1(soft,constant,gauss,forceddist,fold)
...
if nargin < 4, forceddist = 500000; end % month to consider
if nargin < 5, fold = 1; end % 500 km isolation
...
elseif gauss == 1
    ...
    for i = 1:size(cMS,1)
        ...
        idxdist = sqrt( (cMS(i,1)-ch(:,1)).^2 + (cMS(i,2)-ch(:,2)) ) > forceddist;
        
        idx = cMS(i,1) == ch(:,1) & cMS(i,2) == ch(:,2) & idxtemp;
        ck = ch(idx,:);
        ch_nonXval = ch(~idx & idxdist,:);
        zh_mrmv_nonXval = zh_mrmv(idxdist,:);
        tic
...
% saving data
if gauss == 1
    save(sprintf('../matfiles/Xvalforcediso_LOOCV_mon%d_%s%s%s_test1.mat',fold,softstr,constr,gaussstr), ...

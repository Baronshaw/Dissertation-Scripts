function [] = getXvalStatistics_10fold_new(soft,constant,gauss)
...
for i = 1:length(uniyr)
    ...
    % obs vs est
    figure; hold on;
    plot(zhall(idx),zkall(idx),'b.');
    title(sprintf('Daily Year=%d,r2=%0.2f',uniyr(i),r2_year(i)));
    print(gcf,'-painters','-dpng','-r600',sprintf('Daily_Year_%d.png',uniyr(i)));
    
    % locations of high bias
    % maybe have color plot of the number of times there is high relative bias
    % maybe color plot of the mean/median high relative bias
    aberrsub = aberr(idx);
    idxerr = aberrsub >= prctile(aberr,90);
    cksub = ckall(idx,:);
    figure; hold on;
    plot(cksub(:,1),cksub(:,2),'bo',cksub(idxerr,1),cksub(idxerr,2),'ro');
    print(gcf,'-painters','-dpng','-r600',sprintf('LocErr_Year_%d.png',uniyr(i)));
    
%     % color plot of number of times there is a high rel. bias
%     figure; hold on;
%     unihighloc = unique(cksub(idxerr,1:2),'rows');
%     for j = 1:length(unihighloc)
%         idxcount = unihighloc(j,1) == cksub(idxerr,1) & unihighloc(j,2) == cksub(idxerr,2);
%         unihighcount(j) = sum(idxcount);
%     end
%     Property={'Marker','MarkerSize','MarkerEdgeColor'};
%     Value ={'o',5,[0 0 0]};
%     colorplot(unihighloc,unihighcount','hot',Property,Value);
%     cax = [0 50];
%     caxis(cax);
%     colorbar;
    ...
    % times of high bias
    figure; hold on;
    plot(cksub(:,3),aberrsub,'bo',cksub(idxerr,3),aberrsub(idxerr),'ro');
    print(gcf,'-painters','-dpng','-r600',sprintf('TimeErr_Year_%d.png',uniyr(i)));
    
    % which day has the highest number of high biases
    ...
    Property={'Marker','MarkerSize','MarkerEdgeColor'};
    Value ={'o',5,[0 0 0]};
    colorplot(uniIDyr,RMSE_IDyr{i}','hot',Property,Value);
    colorbar;
    print(gcf,'-painters','-dpng','-r600',sprintf('RMSE_Year_%d.png',uniyr(i)));
...
% save results
save(sprintf('Xval_10fold%s%s%s_results.mat',softstr,constr,gaussstr),...
    ...
    'ckyrs','zhyrs','zkyrs','vkyrs','foldyrs','EPAyrs'); % 'dists_info_sID',...

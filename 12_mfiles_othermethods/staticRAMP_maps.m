...
% bsub -x -q day -n 12 -R "span[hosts=1]" /nas02/apps/matlab-2012b/matlab -nodesktop -nosplash -singleCompThread -r "staticRAMP_maps" -logfile "runall_Cluster12.out"
...
% loading data
load(sprintf('matdata/estimation_RAMP_foriso%dkm_timezone.mat',0));
...
for i = 1:length(unidates)
    ...
    title(sprintf('%s static RAMP PM_{2.5} (\\mug/m^3) on %s',var_str,datestr(unidates(i))));
    ...
    print(gcf,'-painters','-dpng','-r600',sprintf('maps/BME_staticRAMP_maps_%s%s%s.png',var_str2,datestr(unidates(i)),est_what_str));

    drawnow;
    frameI = getframe(gcf);
    imI = frame2im(frameI);
    [imindI,cmI] = rgb2ind(imI,256);
    outfileI = sprintf('maps/BME_maps_staticRAMP_%s%s.gif',var_str2,est_what_str);
    ...
    if est_what == 1 & mORv == 1
        ...
        set(gcf,'PaperPosition',[0 0 800 600]./100);
        set(gcf,'PaperPositionMode','manual');
        print(gcf,'-painters','-dpng','-r600',sprintf('maps/Obs_maps_staticRAMP_%s%s.png',datestr(unidates(i)),est_what_str));
        
        drawnow;
        frameII = getframe(gcf);
        imII = frame2im(frameII);
        [imindII,cmII] = rgb2ind(imII,256);
        outfileII = sprintf('maps/Obs_maps_staticRAMP_%s.gif',est_what_str);
        % On the first loop, create the file. In subsequent loops, append.
        if i == 1

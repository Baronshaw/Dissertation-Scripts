function [] = spacetimeDSXvalforcedisolation_LOOCV(additive,multiplicative,forceddist)
...
if nargin < 3, forceddist = 0; end % units = meters
...
for i = 1:len
    ...
    idxdist = sqrt( (unich(i,1)-ch(:,1)).^2 + (unich(i,2)-ch(:,2)).^2 ) > forceddist;
    chX = [ch(~idx&idxdist,:) cht(~idx&idxdist)];
    zh_mtr = zho_mtr(~idx&idxdist);
...
% saving LOOCE
save(sprintf('matdata/LOOCE_DSforcedisolation_%dkm_add_%s_muli_%s.mat', ...
    floor(forceddist./1000),additive,multiplicative), ...

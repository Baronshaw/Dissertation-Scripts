...
gaussstr = '_gauss'; 
forceddist = 0;
load(sprintf('matdata/Xvalforcediso_LOOCV_%s%s%s_foriso%dkm_timezone.mat',softstr,constr,gaussstr,floor(forceddist./1000)));
...
% save Xval results
save('matdata/LOOCE_RAMP_table.mat','ckall_Xval','zh_Xval','zk_Xval','vk_Xval', ...
...
outid = fopen('tables/Xval_RAMP_overall.csv','w+');
fprintf(outid,'%s',strval);
fclose(outid);
dlmwrite('tables/Xval_RAMP_overall.csv',n,'delimiter',',','precision',6,'-append','roffset',1)
...
outid = fopen('tables/Xval_RAMP_daily.csv','w+');
fprintf(outid,'%s',strval);
fclose(outid);
dlmwrite('tables/Xval_RAMP_daily.csv',n,'delimiter',',','precision',6,'-append','roffset',1)

function [] = staticDSXvalforcedisolation_LOOCV(forceddist)
...
parfor i = 1:len
    ...
    zhall{i} = NaN*ones(length(chsub),1);
    for j = 1:length(chsub)
        ...
        idxdist = sqrt( (ch(j,1)-ch(:,1)).^2 + (ch(j,2)-ch(:,2)).^2 ) > forceddist;
        chX = ch(idxdist,:);
        zhX = zh(idxdist);
        [zk_mtr{i}(j),vk{i}(j)]=krigingME_DS(ck,chX,cs,zhX,zs,vs,covmodel,covparam, ...
...
% saving LOOCE
save(sprintf('matdata/LOOCE_DSforcedisolation_%dkm.mat',floor(forceddist./1000)), ...
    'zk_mtr','zk','vk','ckall','unidates','zhall');

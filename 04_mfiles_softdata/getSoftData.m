function [meanGivMod,varGivMod,valMod,valObs,perctile_data,numpnts,...
    idxtME,idxcMS,modplots,dailyCTMg,mean_Mod,mean_Obs,var_Mod,var_Obs,cMSCTM,cMSCTMv, ...
    cMSObsx,cMSObsy,tMEObs] ...
    = getSoftData(dayWHI,n,deltaT,numbins,minpnts,modplots,negval, ...
...
for j = 1:numbins
    ...
    var_Mod(:,j) = nanvar(valModtemp,0,2);
...
if length(dailyCTMg)~=length(cMSCTM)    
    ...
    mean_Mod = mean_Mod(lib,:);
    mean_Obs = mean_Obs(lib,:);
else

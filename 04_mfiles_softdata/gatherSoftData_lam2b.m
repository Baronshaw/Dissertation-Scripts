...
for j = 2001:2002%[2001 2002 2005 2006 2007]
    ...
    for i = 1:r
        ...
        varGivObs = NaN*ones(length(dailyCTMg),1);
        for k = 1:length(meanGivMod)
            idx = ~isnan(mean_Mod(k,:)); % sometimes there's not data between given percentiles
            varGivObs(k) = interp1(mean_Mod(k,idx),var_Mod(k,idx),dailyCTMg(k),'linear','extrap');
        end
        lambda2{i,1} = abs(varGivMod(:,1)) .* abs(varGivObs); % no there's no zeros 
    ...
    save(sprintf('../matfiles/PM2p5_soft_yr%d_lam2b.mat',j),'css','lambda1','lambda2','limi','probdens');

function [meanGivMod,varGivMod,valMod,valObs,perctile_data,numpnts,...
    ...
    = getSoftData_test3(dayWHI,n,deltaT,numbins,minpnts,modplots,negval, ...
...
for j = 1:numbins
    ...
    timedists  = timeweightObstemp;
    spacedists = distweightObstemp;
    ...
    % test 3 - I think that temporal variation is important, so I'll weight
    % the points of the bin by temporal distance
    at = 200; % exponential decay of 200 days
    wi = exp( - timeweightObstemp./at );
    mean_Obs(:,j) = nansum(wi.*valObstemp,2) ./ nansum(wi,2);
    var_Obs(:,j) = nansum(wi.*(valObstemp-repmat(mean_Obs(:,j),1,c)).^2,2) ./ nansum(wi,2);

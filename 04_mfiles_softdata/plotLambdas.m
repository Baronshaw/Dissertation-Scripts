function [] = plotLambdas(dayWHI,daysWHIdisp,n,deltaT,numbins,minpnts,modplots,...
    negval)
...
%%% lambda1
if length(meanGivMod) ~= 0 % if soft data exist
    ...
    if size(CTMlocs,1) == size(meanGivMod(:,1),1)
        ...
        [xg yg Zg] = plotField(cMSCTMv,meanGivMod(:,1),lax,[plotax(:,1) plotax(:,2)]);
    end
    colorbar;
    cax = [2 30];
    caxis(cax);
    ...
    set(gca,'XTickLabel',get(gca,'XTick')/1000);
    set(gca,'YTickLabel',get(gca,'YTick')/1000);
    print(gcf,'-painters','-dpdf','-r600',sprintf('../plots/map_lambda1_%d_%d_%d_%d_%d.pdf',daysWHIdisp,deltaT,n,minpnts,numbins));
    ...
    if size(CTMlocs,1) == size(meanGivMod(:,1),1)
        ...
        [xg yg Zg] = plotField(cMSCTMv,varGivMod(:,1),lax,[plotax(:,1) plotax(:,2)]);
    end
    colorbar;
    ...
    set(gca,'XTickLabel',get(gca,'XTick')/1000);
    set(gca,'YTickLabel',get(gca,'YTick')/1000);
    print(gcf,'-painters','-dpdf','-r600',sprintf('../plots/map_lambda2_%d_%d_%d_%d_%d.pdf',daysWHIdisp,deltaT,n,minpnts,numbins));

...
% with all metrics, high numbers are bad except with R/R2
...
for i = 1:length(valplot)
    for j = 1:length(yrznum)
        ...
        print(gcf,'-painters','-dpng','-r600',sprintf('figures/%s_%s_grid.png',strnm{i},datestr(yrznum(j,:))));

        drawnow;
        frameI = getframe(gcf);
        imI = frame2im(frameI);
        [imindI,cmI] = rgb2ind(imI,256);
        outfileI = sprintf('figures/%s_grid.gif',strnm{i});
